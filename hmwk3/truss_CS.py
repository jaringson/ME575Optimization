...
def truss(A):
    ...
    for i in range(A.shape[0]):
        ...
        Xim = deepcopy(A.astype(complex))
        Xim[i] = np.complex(A[i],perturb)
        mass_im, stress_im = _truss(Xim)
        # set_trace()

        mass_gradient[i] = np.imag(mass_im)/perturb
        stress_gradient[i] = np.imag(stress_im)/perturb

    return mass, mass_gradient, stress, stress_gradient.T
...
def _truss(A):
    ...
    K = np.zeros((DOF * n, DOF * n), dtype=A.dtype)
    S = np.zeros((nbar, DOF * n), dtype=A.dtype)
